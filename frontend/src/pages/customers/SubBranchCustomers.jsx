...
export default function SubBranchCustomers() {
  ...
  useEffect(() => {
    if (!branch) return;
    setLoading(true);
    fetchCustomers({ type: "sub-branch", branch })
      ...
      <div className="page-header">
        <div>
          <h1 className="page-title flex items-center gap-2">
            <UserCheck size={22} className="text-emerald-500" /> Sub-Branch Customers
          </h1>
          <p className="page-sub">Filter customers by sub-branch</p>
        </div>
      </div>
        ...
        <div className="erp-card overflow-hidden animate-slideInRight">
          {loading ? (
            ...
            <table className="erp-table">
              <thead>
                <tr><th>#</th><th>Customer</th><th>Branch</th><th>Contact</th></tr>
              </thead>
              <tbody>
                {customers.length === 0 ? (
                  <tr><td colSpan={4} className="py-12 text-center text-slate-400">No customers found.</td></tr>
                ) : customers.map((c, i) => (
                  <tr key={c.id}>
