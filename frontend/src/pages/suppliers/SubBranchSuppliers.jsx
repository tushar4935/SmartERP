...
export default function SubBranchSuppliers() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetchSuppliers({ type: "sub-branch", branch })
      ...
      <div className="page-header">
        <div>
          <h1 className="page-title flex items-center gap-2">
            <Truck size={22} className="text-orange-500" /> Sub-Branch Suppliers
          </h1>
          <p className="page-sub">Filter suppliers by main branch name</p>
        </div>
      </div>

      <div className="erp-card p-5 animate-slideInLeft">
        <label className="text-sm font-medium text-slate-700 mb-2 flex items-center gap-2">
          <Search size={15} /> Main Branch Name
        </label>
        <input
          value={branch}
        ...
        <div className="erp-card overflow-hidden animate-slideInRight">
          {loading ? (
            ...
            <table className="erp-table">
              <thead>
                <tr><th>#</th><th>Supplier</th><th>Company</th><th>Contact</th><th>Actions</th></tr>
              </thead>
              <tbody>
                {data.length === 0 ? (
                  <tr><td colSpan={5} className="py-12 text-center text-slate-400">No suppliers found.</td></tr>
                ) : data.map((s, i) => (
                  <tr key={s.id}>
